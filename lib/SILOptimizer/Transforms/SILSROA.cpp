...
createAllocas(llvm::SmallVector<AllocStackInst *, 4> &NewAllocations) {
  ...
  if (TT) {
    ...
    for (VarDecl *VD : SD->getStoredProperties()) {
      ...
      if (VarInfo) {
        NewDebugVarInfo = VarInfo->copyVariable();
        if (!NewDebugVarInfo->Type)
          NewDebugVarInfo->Type = Type;
        if (!NewDebugVarInfo->Loc)
          NewDebugVarInfo->Loc = InstLoc;
        if (!NewDebugVarInfo->Scope)
          NewDebugVarInfo->Scope = InstDS;

        // TODO: Handle existing DIExpr attached on VarInfo
