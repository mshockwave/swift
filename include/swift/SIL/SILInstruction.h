...
protected:
  ...
  SILDebugVariableSupplement(unsigned NumDIExprOps, bool AuxType,
                             bool AuxLoc, bool AuxScope)
    : NumDIExprOperands(NumDIExprOps), HasAuxDebugVariableType(AuxType),
      AuxVariableSourceLoc((AuxLoc ? SourceLocKind::Loc : 0) |
                           (AuxScope ? SourceLocKind::Scope : 0)) {}
};

#define SIL_DEBUG_VAR_SUPPLEMENT_TRAILING_OBJS_IMPL() \
  inline bool hasAuxDebugLocation() const {  \
    return AuxVariableSourceLoc & SourceLocKind::Loc;  \
  }  \
  inline bool hasAuxDebugScope() const {  \
    return AuxVariableSourceLoc & SourceLocKind::Scope;  \
  }  \
  \
...
public:
  ...
  Optional<SILDebugVariable> getVarInfo() const {
    ...
    return VI.get(getDecl(), getTrailingObjects<char>(), AuxVarType,
